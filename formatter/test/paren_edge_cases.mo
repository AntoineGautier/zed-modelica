...
model ParenEdgeCases
  ...
  parameter Real test1 =
    if outer_cond
    then (if inner_var == VeryLongPackageName.SubModule.AnotherModule.TypeDefinition.EnumValue
      then result_when_true
  ...
  parameter Real test3 =
    if cond1
    then ((if cond2 == Value.A then x else y) + (if cond3 == Value.B then a else b))
    else default;
  ...
  parameter Boolean test7 =
    if condition_a and (if condition_b == Enum.Type.Value then condition_c else condition_d)
    then true
    else false;
  ...
  parameter Real test8 =
    if has_array
    then array_data[(if index_type == System.IndexType.Primary then primary_index else secondary_index)]
    else default;
  ...
  parameter Real test10 =
    if outer
    then (if flag and variable == LongPackage.Module.Type.EnumValue or other_flag
      then result_a
      else result_b)
    else result_c;
end ParenEdgeCases;